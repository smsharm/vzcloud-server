...
public class DashboardCotroller {
    ...
    /**
     * Query the host list according to the conditions
     *
     * @param model
     * @param request
     * @return
    ...
    public String mainList(Model model, HttpServletRequest request) {
        ...
        try {
            ...
            memPerSize_90_chart.setItem("Memory>90%");
            ...
            memPerSize_50_90_chart.setItem("Memory> 50%and <90%");
            ...
            perSize_90_90_chart.setItem("CPU and memory>90%");
            ...
            perSize_50_50_chart.setItem("CPU and memory<50%");
            ...
            logger.error("Main panel information abnormal:", e);
            logInfoService.save("dash/main", "Main panel information error:" + e.toString(), StaticKeys.LOG_ERROR);
        }
    ...
    public String systemInfoList(SystemInfo systemInfo, Model model, HttpServletRequest request) {
        ...
        try {
            ...
            //Set the total use rate of disk begin
            for (SystemInfo systemInfo1 : pageInfo.getList()) {
                ...
                try {
                    ...
                    logger.error("Set the total usage rate error", e);
                }
            }
            //Set the total use rate of disk end

            PageUtil.initPageNumber(pageInfo, model);
            model.addAttribute("pageUrl", "/dash/systemInfoList?1=1" + url.toString());
            model.addAttribute("page", pageInfo);
        } catch (Exception e) {
            logger.error("Query server list error:", e);
            logInfoService.save("Query server list error", e.toString(), StaticKeys.LOG_ERROR);
        }
        if (request.getParameter(StaticKeys.DASH_VIEW_ACCOUNT) != null) {
    ...
    /**
     * Inquiry server details according to the IP query server
     *
     * @param model
     * @param request
     * @return
    ...
    public String hostDetail(Model model, HttpServletRequest request) {
        ...
        try {
            ...
            logger.error("Server details error:", e);
            logInfoService.save(hostname, "View server details error", e.toString());
        }
        if (request.getParameter(StaticKeys.DASH_VIEW_ACCOUNT) != null) {
    ...
    public String delete(Model model, HttpServletRequest request, RedirectAttributes redirectAttributes) {
        String errorMsg = "Delete the host information error:";
        try {
            if (!StringUtils.isEmpty(request.getParameter("id"))) {
                ...
                for (String id : ids) {
                    ...
                    logInfoService.save("Delete the host:" + sys.getHostname(), sys.getHostname(), StaticKeys.LOG_ERROR);
                }
    ...
    /**
     * According to the IP query server graphics report
     *
     * @param model
     * @param request
     * @return
    ...
    public String hostChart(Model model, HttpServletRequest request) {
        ...
        try {
            ...
            logger.error("Server graphics report error:", e);
            logInfoService.save(hostname, "Graphical report error", e.toString());
        }
        if (request.getParameter(StaticKeys.DASH_VIEW_ACCOUNT) != null) {
