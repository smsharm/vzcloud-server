...
import com.wgcloud.config.MqttConfig;
...
public class AgentService {
  ...
  @Resource private SystemInfoService systemInfoService;
  ...
  private final ThreadPoolExecutor executor =
      new ThreadPoolExecutor(10, 40, 2, TimeUnit.MINUTES, new LinkedBlockingDeque<>());
  ...
  public void init() throws MqttException {
    ...
    MqttClient client =
        new MqttClient(mqttConfig.getBrokerUrl(), clientId, new MemoryPersistence());
    client.connect(connectOptions);
    client.subscribeWithResponse(
        mqttConfig.getMinTaskTopic(),
        (topic, msg) -> {
          handle(String.valueOf(msg));
        });
  }

  public JSONObject handle(String data) {
    JSONObject agentJsonObject = (JSONObject) JSONUtil.parse(data);
